...
void ToyFit(int bin = 1){
  ...
  int binhigh = 0;
  if(bin==0){
    binlow = 150; binhigh = 450;
  }
  if(bin==1){
    binlow = 50; binhigh = 250;
  }
  if(bin==2){
    binlow = 0; binhigh = 100;
  }
  if(bin==3){
    binlow = 0; binhigh = 100;
  }
...
//  TFile *inf = new TFile("../ToMerge20140408/SigmaBplus.root");//default
//  TFile *inf = new TFile("../ToMerge20140409/SigmaBplus.root");//poisson sig
//  TFile *inf = new TFile("../ToMerge20140416/SigmaBplus.root");//updated
//  TFile *inf = new TFile("../ToMerge20140426/SigmaBplus.root");//updated
  TFile *inf = new TFile("../ToMerge20140428/SigmaBplus.root");//
...
//	true_y = new TH1D("true_y","yields",20,150,500);                                   
  ...
  if(bin==2)  leg->AddEntry(toys,Form("pt 20~25, true#: %.2f", true_y->GetMean()),"");
  if(bin==3)  leg->AddEntry(toys,Form("pt 25~30, true#: %.2f", true_y->GetMean()),"");
  if(bin==4)  leg->AddEntry(toys,Form("pt 30~60, true#: %.2f", true_y->GetMean()),"");
  leg->Draw();
...
//      if(bin==0 )pull->Fill((yield-281.464)/yield_err);
//      if(bin==1 )pull->Fill((yield-142.753)/yield_err);
//      if(bin==2 )pull->Fill((yield-52.0226)/yield_err);
//      if(bin==3 )pull->Fill((yield-26.572)/yield_err);
//      if(bin==4 )pull->Fill((yield-30.7691)/yield_err);
